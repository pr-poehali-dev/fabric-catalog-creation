...
const FABRICS = [
  ...
  },
  {
    ...
    details: {
      ...
      origin: "Россия",
      careInstructions: "Машинная стирка при 30°C, не использовать отбеливатель"
    },
    features: [
      "Прочность",
      "Легкость в уходе",
  ...
  },
  {
    ...
    details: {
      ...
      composition: "100% хлопок",
      origin: "Италия",
      careInstructions: "Машинная стирка при 30°C, стирать с изнаночной стороны"
    },
    features: [
      "Высокая прочность",
      "Износостойкость",
...
const FabricDetail = () => {
  ...
  if (!fabric) {
    return (
      <div className="container mx-auto max-w-6xl px-4 py-12 text-center">
        <Icon name="FileQuestion" className="mx-auto h-16 w-16 text-muted-foreground mb-4" />
        <h1 className="text-2xl font-bold mb-4">Ткань не найдена</h1>
        <p className="text-muted-foreground mb-6">К сожалению, запрашиваемая ткань не существует или была удалена.</p>
  ...
  const recommendations = FABRICS.filter(f => 
    f.category === fabric.category && f.id !== fabric.id
  ).slice(0, 3);
      ...
      <Tabs defaultValue="details" className="mb-12">
        ...
        <TabsContent value="details" className="bg-muted/30 p-6 rounded-lg">
          ...
          <p className="text-muted-foreground mb-4">
            {fabric.description} Это качественный материал, который отлично подойдет для
            различных проектов. Ниже приведены инструкции по уходу и дополнительная информация.
          </p>
          
          <h3 className="text-xl font-semibold mt-6 mb-3">Инструкции по уходу</h3>
          ...
          <ul className="space-y-2">
            {fabric.features.map((feature, index) => (
              <li key={index} className="flex items-center">
                <Icon name="CheckCircle" className="h-5 w-5 text-primary mr-2" />
                {feature}
              </li>
            ))}
          </ul>
        ...
        <TabsContent value="applications" className="bg-muted/30 p-6 rounded-lg">
